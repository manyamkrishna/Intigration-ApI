...
import styles from "../../../addproducts/add-laptop/products.module.css";
...
const page = ({ params }) => {
  ...
  const getProductHandler = async () => {
    const response = await fetch(
      `http://localhost:3000/api/products/laptops/update/${id}`
    );
  ...
  const productUpdateHandler = async (e) => {
    ...
    const response = await fetch(
      `http://localhost:3000/api/products/laptops?id=${id}`,
      {
        method: "PUT",
        body: JSON.stringify({
          newTitle: name,
